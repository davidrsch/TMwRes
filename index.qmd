...
Welcome to [*Tidy Modeling with R*](https://amzn.to/35Hn96s)! This book is a guide to using a collection of software in the R programming language for model building called `r pkg(tidymodels)`, and it has two main goals:
...
In [Chapter -@sec-software-modeling], we outline a taxonomy for models and highlight what good software for modeling is like. The ideas and syntax of the tidyverse, which we introduce (or review) in [Chapter -@sec-tidyverse], are the basis for the tidymodels approach to these challenges of methodology and practice. [Chapter -@sec-base-r] provides a quick tour of conventional base R modeling functions and summarizes the unmet needs in that area.

After that, this book is separated into parts, starting with the basics of modeling with tidy data principles. Chapters -@sec-ames through -@sec-performance introduces an example data set on house prices and demonstrates how to use the fundamental tidymodels packages: `r pkg(recipes)`, `r pkg(parsnip)`, `r pkg(workflows)`, `r pkg(yardstick)`, and others.
