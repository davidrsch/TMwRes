...
# Screening Many Models {#sec-workflow-sets}

We introduced workflow sets in [Chapter @sec-workflows] and demonstrated how to use them with resampled data sets in [Chapter @sec-compare]. In this chapter, we discuss these sets of multiple modeling workflows in more detail and describe a use case where they can be helpful.
...
Workflow sets provide a user interface to create and manage this process. We'll also demonstrate how to evaluate these models efficiently using the racing methods discussed in @sec-racing-example.
...
The `autoplot()` method plots the rankings; it also has a `select_best` argument. The plot in @fig-workflow-set-ranks visualizes the best results for each model and is generated with:
...
In case you want to see the tuning parameter results for a specific model, like @fig-workflow-sets-autoplot, the `id` argument can take a single value from the `wflow_id` column for which model to plot:
...
## Efficiently Screening Models {#sec-racing-example}

One effective method for screening a large set of models efficiently is to use the racing approach described in @sec-racing. With a workflow set, we can use the `workflow_map()` function for this racing approach. Recall that after we pipe in our workflow set, the argument we use is the function to apply to the workflows; in this case, we can use a value of `"tune_race_anova"`. We also pass an appropriate control object; otherwise the options would be the same as the code in the previous section.
...
The same helpful functions are available for this object to interrogate the results and, in fact, the basic `autoplot()` method shown in @fig-workflow-set-racing-ranks[^15-workflow-sets-1] produces trends similar to @fig-workflow-set-ranks. This is produced by:

[^15-workflow-sets-1]: As of February 2022, we see slightly different performance metrics for the neural network when trained using macOS on ARM architecture (Apple M1 chip) compared to Intel architecture.
...
Did we get similar results? For both objects, we rank the results, merge them, and plot them against one another in @fig-racing-concordance.
...
We can see the test set metrics results, and visualize the predictions in @fig-concrete-test-results.
...
## Chapter Summary {#sec-workflow-sets-summary}
