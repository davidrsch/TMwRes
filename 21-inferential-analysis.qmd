...
::: rmdnote
In @sec-model-types, we outlined a taxonomy of models and said that most models can be categorized as descriptive, inferential, and/or predictive.
:::
...
We will use p-values in this chapter. However, the tidymodels framework tends to promote confidence intervals over p-values as a method for quantifying the evidence for an alternative hypothesis. As previously shown in @sec-tidyposterior, Bayesian methods are often superior to both p-values and confidence intervals in terms of ease of interpretation (but they can be more computationally expensive).
...
The `r pkg(infer)` package has a high-level API for showing the analysis results, as shown in @fig-bootstrapped-mean.
...
Since the interval visualized in in @fig-bootstrapped-mean does not include zero, these results indicate that men have published more articles than women.
...
The following visualization code is also very similar to the bootstrap approach. This code generates @fig-permutation-dist where the vertical line signifies the observed value:
...
The vertical line representing the null hypothesis in @fig-permutation-dist is far away from the permutation distribution. This means, if in fact the null hypothesis were true, the likelihood is exceedingly small of observing data at least as extreme as what is at hand.
...
::: rmdwarning
When we compare these results (in @fig-glm-intervals) to the purely parametric results from `glm()`, the bootstrap intervals are somewhat wider. If the data were truly Poisson, these intervals would have more similar widths.
:::
...
It's a good idea to visualize the bootstrap distributions of the coefficients, as in @fig-zip-bootstrap.
...
## More Inferential Analysis {#sec-inference-options}
...
## Chapter Summary {#sec-inference-summary}
