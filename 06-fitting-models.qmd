...
El paquete `r pkg(parsnip)`, un de los paquetes de R que es parte del metapaquete `r pkg(tidymodels)`, proporciona una interfaz fluida y estandarizada para una variedad de modelos diferentes. En este capítulo, damos algunas motivaciones sobre por qué una interfaz común es beneficiosa para comprender y construir modelos en la práctica y mostramos cómo usar el paquete `r pkg(parsnip)`.
...
::: rmdnote
Proporcionamos un argumento de `penalty` (penalización) requerido para el motor glmnet. Además, para los motores Stan y glmnet, el argumento `family`(familia) se agregó automáticamente como valor predeterminado. Como se mostrará más adelante en esta sección, esta opción se puede cambiar.
:::
...
En un esfuerzo por hacer que la especificación de argumentos sea menos complicada, `r pkg(parsnip)` utiliza nombres de argumentos comunes dentro y entre paquetes. @tbl-parsnip-args muestra, para random forest, qué utilizan los modelos `r pkg(parsnip)`.
